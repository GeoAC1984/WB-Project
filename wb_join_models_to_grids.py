...
grids_path=os.path.join(server,'Regions')

regions=os.listdir(grids_path)
regions_w_countries=[r for r in regions if os.path.isdir(os.path.join(grids_path,r)) and os.listdir(os.path.join(grids_path,r))!=[]]

CropModels=[m for m in os.listdir(os.path.join(server,'Glob_Points','CropModels_8dec')) if m.endswith('shp')]
WaterModels=[m for m in os.listdir(os.path.join(server,'Glob_Points','WaterModels_8dec')) if m.endswith('shp')]
...
for region in regions_w_countries:
    ...
    for country in os.listdir(os.path.join(grids_path,region)):
        if country=='.DS_Store':
            ...
            cell05=[c for c in shapes if '0_5' in c][0]
            cell025=[c for c in shapes if '0125' in c][0]
            ...
            for water_model in WaterModels:
                ...
                model=gpd.read_file(os.path.join(server, 'Glob_Points','WaterModels_8dec',water_model))
                try:
            ...
            for crop_model in CropModels:
                ...
                model=gpd.read_file(os.path.join(server,'Glob_Points','CropModels_8dec',crop_model))
                try:                
                    data_grid05=gpd.sjoin(shp05,model, how='left',op="contains").drop(['index_right','ID',],1)       
                    centroids_data=gpd.sjoin(centers,data_grid05, how='left',op='within').drop(['index_right'],1)
                    data_grid025=gpd.sjoin(shp025,centroids_data, how='left',op='intersects').drop(['index_right'],1)
