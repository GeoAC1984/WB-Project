        ...
        if os.listdir(os.path.join(grids_path,region,country,'WaterOutput'))==[]:
            ...
            for water_model in WaterModels:
                ...
                try:
                    ...
                    print "There is no ", water_model," points for this country"
                    break
            for crop_model in CropModels:
                ...
                try:                
                    ...
                    print "There is no ", crop_model," points for this country"
                    break
        else:
            print  country,' in the ',region, ' region is completed'
=======
regions=os.listdir(grids_path)## all the regions 
regions_w_countries=[r for r in regions if os.listdir(os.path.join(grids_path,r))!=[]]## regions with input grids
##regions_w_countries.pop(1) ## remove CentralAmerica; was completed during the testing

CropModels=[m for m in os.listdir(r'T:\WorldBankProject\Glob_Points\CropModels_8dec') if m.endswith('shp')]## list of CropModels (all 8 decades in one shp)
WaterModels=[m for m in os.listdir(r'T:\WorldBankProject\Glob_Points\WaterModels_8dec') if m.endswith('shp')]## list of WaterModels (all 8 decades in one shp)
    ...
    for country in os.listdir(os.path.join(grids_path,region)):
        ...
        for water_model in WaterModels:
            ...
            try:
                ...
                print "There is no ", water_model," points for this country" ## the join will throw an error in cases where country is small (1-2 0.5 grid cells)
                break                                                                                ## and no model points fall within those cells 
        for crop_model in CropModels:
            shape_out=country_prefix+'_'+crop_model
            model=gpd.read_file(os.path.join(r'T:\WorldBankProject\Glob_Points\CropModels_8dec',crop_model))
            try:                
